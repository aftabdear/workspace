		...
		if (Areas.lumbridgeTowerFloor0.contains(script.myPlayer().getPosition())) {
			if (door != null && door.isVisible()) {
				if (door.hasAction("Open")) {
					...
					Position[] pathFromLadderToStairFloor = {
						    new Position(3225, 3223, 0),
						    new Position(3221, 3218, 0),
						    new Position(3214, 3219, 0),
						    new Position(3214, 3227, 0),
						    new Position(3206, 3228, 0)
						};

					if (script.getWalking().walkPath(Arrays.asList(pathFromLadderToStairFloor))) {
						Sleep.sleepUntil(() -> Areas.lumbridgeStairsFloor0North.contains(script.myPlayer().getPosition()),
								5000);
					}
				}
			}
