...
public class Config5 extends Task {

	public Config5(Script script) {
		super(script);
	...
	public boolean verify() {
		...
		return script.getConfigs().get(144) == 60;
	}

	@Override
	public int execute() throws Exception {
		script.log("in config5 class - walking to romeo");
		...
		Position outside = new Position(3166,3433,0);
		...
		if (Areas.firstFloorOfJulietsHouse.contains(script.myPlayer().getPosition())){
			...
			if (doorOutsideOpen != null) {
				WalkingEvent wEvent = new WalkingEvent(outside);
