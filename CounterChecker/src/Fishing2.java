...
public class Fishing2 extends Task {

	// to change back to normal theme window -> preferences-> general ->
	// appearances

	public Fishing2(Script script) {
		super(script);
	...
	public boolean verify() {
		...
		return script.getSkills().getDynamic(Skill.FISHING) >= 20 && script.getSkills().getDynamic(Skill.FISHING) <= 39
	...
	public int execute() throws Exception {
		...
		if (Areas.lumbridgeFishingSpot2.contains(script.myPlayer().getPosition())) {
			...
			if (script.myPlayer().getAnimation() == -1) {
				...
				if (script.getSettings().isShiftDropActive() == false){
				...
				else if (fishingSpot != null && fishingSpot.isVisible()) {
					...
					script.getWalking().walk(Areas.lumbridgeFishingSpotFailSafe2);
				}
